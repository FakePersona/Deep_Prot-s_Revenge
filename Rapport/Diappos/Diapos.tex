...
\begin{frame}{Peu de travaux concernant les protéines}
  ...
  %   for anyone how does not follow everything on the slide itself.
    ...
    \item Prédiction de structures secondaires\pause
      % \begin{itemize}
      % \item Heffernan R, Paliwal K, Lyons J et al. 2015 Improving prediction of secondary structure, local backbone
      %   angles, and solvent accessible surface area of proteins by iterative deep
      %   learning. 
      % \item Spencer M, Eickholt J, Cheng J. 2015 A Deep Learning Network Approach to ab initio Protein
      %   Secondary Structure Prediction
      % \end{itemize}
    \item Classification de protéines selon différents critéres\pause
      % \begin{itemize}
      % \item Hochreiter S, Heusel M, Obermayer K. Fast model-based protein homology detection without
      %   alignment. 2007 \alert{1 seule couche récurrente}
      % \end{itemize}
    \item Prédiction de contacts
      % \begin{itemize}
      % \item Lena PD, Nagata K, Baldi P. 2012 Deep architectures for protein contact map prediction. 
      % \end{itemize}
    \end{itemize}
  \end{columns}
