...
Properties = np.zeros(((len(data)**2  - len(data))//2, 55)) 
k = 0
...
for i in range(len(data)):
    for j in range(i+1, len(data)):
        ...
        # Alignment scores
        Properties[k][51] = pairwise2.align.globalxx(data[i],data[j], score_only=1)
        Properties[k][52] = pairwise2.align.localxx(data[i],data[j], score_only=1)
        Properties[k][53] = pairwise2.align.globaldx(data[i],data[j], matrix, score_only=1) #BLOSUM!
        # Structural score
        ...
        Properties[k][54] = super_imposer.rms
        k += 1
